...
void validate_gemm_oracle() {
  ...
  for (int column = 0; column < columns; ++column) {
    input[column] = __float2half_rn(
        std::sin(static_cast<float>(column + 1) * 0.071F) * 0.25F);
    for (int row = 0; row < rows; ++row) {
      weights[static_cast<std::size_t>(column) * rows + row] =
          __float2half_rn(static_cast<float>((row * 7 + column * 3) % 19 - 9) /
                          64.0F);
    }
  }
  ...
  for (int row = 0; row < rows; ++row) {
    for (int column = 0; column < columns; ++column) {
      expected[row] +=
          __half2float(weights[static_cast<std::size_t>(column) * rows + row]) *
          __half2float(input[column]);
    }
  }

  check(cudaSetDevice(0), "select oracle device");
  ...
  check(cudaMemcpy(device_weights.as<void>(), weights.data(),
        ...
        "upload oracle weights");
  check(cudaMemcpy(device_input.as<void>(), input.data(),
                   input.size() * sizeof(__half), cudaMemcpyHostToDevice),
        "upload oracle input");

  cublasHandle_t handle{};
  check(cublasCreate(&handle), "create oracle cuBLAS handle");
  const float alpha = 1.0F;
  const float beta = 0.0F;
  const auto status = cublasGemmEx(
      handle, CUBLAS_OP_N, CUBLAS_OP_N, rows, 1, columns, &alpha,
      device_weights.as<__half>(), CUDA_R_16F, rows,
      device_input.as<__half>(), CUDA_R_16F, columns, &beta,
      device_output.as<float>(), CUDA_R_32F, rows, CUBLAS_COMPUTE_32F,
      CUBLAS_GEMM_DEFAULT);
  if (status != CUBLAS_STATUS_SUCCESS) {
    cublasDestroy(handle);
    check(status, "run oracle GEMM");
  }
  ...
  check(cublasDestroy(handle), "destroy oracle cuBLAS handle");
  ...
  if (maximum_error > 2.0e-4 || relative_l2 > 2.0e-5) {
    throw std::runtime_error("independent FP16 GEMM oracle failed");
  }
}
...
class Shard final {
 public:
  Shard(int device, std::uint32_t intermediate)
    ...
    check(cudaSetDevice(device_), "select FP16 shard");
    check(cudaStreamCreateWithFlags(&stream_, cudaStreamNonBlocking),
          "create FP16 shard stream");
    check(cublasCreate(&handle_), "create FP16 cuBLAS handle");
    check(cublasSetStream(handle_, stream_), "bind cuBLAS stream");
    ...
    check(cudaPeekAtLastError(), "initialize FP16 weights");
    check(cudaStreamSynchronize(stream_), "finish FP16 weight initialization");
  }
  ...
  void launch() {
    check(cudaSetDevice(device_), "select launching FP16 shard");
    const float alpha = 1.0F;
    const float beta = 0.0F;
    check(cublasGemmEx(
              handle_, CUBLAS_OP_N, CUBLAS_OP_N,
              static_cast<int>(2U * intermediate_), 1,
              static_cast<int>(kHidden), &alpha,
              gate_up_weights_.as<__half>(), CUDA_R_16F,
              static_cast<int>(2U * intermediate_), input_.as<__half>(),
              CUDA_R_16F, static_cast<int>(kHidden), &beta,
              gate_up_.as<float>(), CUDA_R_32F,
              static_cast<int>(2U * intermediate_), CUBLAS_COMPUTE_32F,
              CUBLAS_GEMM_DEFAULT),
          "launch resident FP16 gate/up");
    swiglu_to_half<<<(intermediate_ + kThreads - 1U) / kThreads, kThreads, 0,
                     stream_>>>(gate_up_.as<float>(),
                                intermediate_buffer_.as<__half>(),
                                intermediate_);
    check(cudaPeekAtLastError(), "launch FP16 SwiGLU");
    check(cublasGemmEx(
              handle_, CUBLAS_OP_N, CUBLAS_OP_N, static_cast<int>(kHidden), 1,
              static_cast<int>(intermediate_), &alpha,
              down_weights_.as<__half>(), CUDA_R_16F,
              static_cast<int>(kHidden), intermediate_buffer_.as<__half>(),
              CUDA_R_16F, static_cast<int>(intermediate_), &beta,
              output_.as<float>(), CUDA_R_32F, static_cast<int>(kHidden),
              CUBLAS_COMPUTE_32F, CUBLAS_GEMM_DEFAULT),
          "launch resident FP16 down");
  }

  void synchronize() {
    check(cudaSetDevice(device_), "select synchronizing FP16 shard");
    check(cudaStreamSynchronize(stream_), "synchronize FP16 shard");
  }
...
class Program final {
 public:
  Program()
    ...
    check(cudaSetDevice(0), "select initial FP16 input device");
    check(cudaMemcpy(shards_[0]->input(), input.data(),
                     input.size() * sizeof(__half), cudaMemcpyHostToDevice),
          "upload initial FP16 input");
  }
 ...
 private:
  void run_layer() {
    ...
    check(cudaPeekAtLastError(), "launch FP16 reduction");
    check(cudaStreamSynchronize(shards_[0]->stream()),
          "complete FP16 reduction");
  }
